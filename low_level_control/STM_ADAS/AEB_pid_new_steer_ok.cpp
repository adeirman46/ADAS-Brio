...
#define ANALOG1 A0               // Analog input for steering
#define ANALOG2 A1               // Analog input for steering
#define DAC1 PA4                 // DAC output for steering
#define DAC2 PA5                 // DAC output for steering
#define POTENTIOMETER_PIN PB0    // Potentiometer for steering angle
...
// Steering Variables
const int IDLE_STATE = 775;      // 2.5V at 10-bit resolution (2.5V / 3.3V * 1023)
static float smoothed_val1 = IDLE_STATE;
static float smoothed_val2 = IDLE_STATE;
const float alpha = 0.05;         // Smoothing factor for low-pass filter
float voltage1 = 2.5;            // Steering DAC1 voltage
float voltage2 = 2.5;            // Steering DAC2 voltage
...
// Function to set voltage for throttle or steering control
...
// Function to set steering in manual mode
void setSteering() {
...
void setup() {
    ...
    analogReadResolution(10); // Set ADC to 10-bit
...
void loop() {
    ...
    if (current_time - last_time >= SAMPLE_TIME_MS) {
        ...
        // Handle steering in manual mode
        setSteering();
