...
// // GPS pins for software serial
// const int GPS_RX = D2;  // Nucleo RX pin for GPS TX
// const int GPS_TX = D3;  // Nucleo TX pin for GPS RX
...
const unsigned long GPS_UPDATE_MS = 1000;  // 1Hz GPS update rate
...
float calculateThrottle(float dt) {
    ...
    error_sum += error * dt;
    error_sum = constrain(error_sum, -10, 10);  // Anti-windup
...
void setup() {
    Serial.begin(9600);    // Main serial for control
    Serial1.begin(9600); // Software serial for GPS
...
void sendGPSData() {
    if (gps.location.isValid()) {
        // Send GPS marker
        Serial.write('G');
        // Send latitude and longitude as floats
        float lat = gps.location.lat();
        float lon = gps.location.lng();
        Serial.write((uint8_t*)&lat, sizeof(float));
        Serial.write((uint8_t*)&lon, sizeof(float));
    }
}
...
void loop() {
    ...
    while (Serial1.available() > 0) {
        if (gps.encode(Serial1.read())) {
            // If there's a new valid GPS fix, check if it's time to send
            if (current_time - last_gps_time >= GPS_UPDATE_MS) {
                sendGPSData();
