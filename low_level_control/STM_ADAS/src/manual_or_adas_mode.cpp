...
// const int steerDacResolution = 4095;    // 12-bit resolution for steering
...
// const int STEER_IDLE_STATE = 3100; // 2.5V on 12-bit scale (2.5/3.3*4095)
// const int STEER_ADC_CENTER = 530;  // Center position for potentiometer
...
//   analogReadResolution(12); // 12-bit for all ADC inputs
//   analogWriteResolution(12); // Default to 12-bit, adjusted in setVoltage
...
//   setVoltage(STEER_OUT1, STEER_CENTER, steerDacResolution);
//   setVoltage(STEER_OUT2, STEER_CENTER, steerDacResolution);

//   // Calculate IIR filter coefficients for throttle
//   float alpha = (throttleWc * throttleTs) / 2.0;
//   float denom = 1.0 + sqrt(2.0) * alpha + alpha * alpha;
//   throttle_a1 = -2.0 * (1.0 - alpha * alpha) / denom;
//   throttle_a2 = (1.0 - sqrt(2.0) * alpha + alpha * alpha) / denom;
//   throttle_b0 = (alpha * alpha) / denom;
//   throttle_b1 = 2.0 * throttle_b0;
//   throttle_b2 = throttle_b0;

//   // Initialize throttle filter with idle assumption
//   float throttle_initial_value = 140; // Assume idle position (140)
...
//   alpha = (brakeWc * brakeTs) / 2.0;
//   denom = 1.0 + sqrt(2.0) * alpha + alpha * alpha;
//   brake_a1 = -2.0 * (1.0 - alpha * alpha) / denom;
//   brake_a2 = (1.0 - sqrt(2.0) * alpha + alpha * alpha) / denom;
//   brake_b0 = (alpha * alpha) / denom;
...
//   alpha = (steerWc * steerTs) / 2.0;
//   denom = 1.0 + sqrt(2.0) * alpha + alpha * alpha;
//   steer_a1 = -2.0 * (1.0 - alpha * alpha) / denom;
//   steer_a2 = (1.0 - sqrt(2.0) * alpha + alpha * alpha) / denom;
//   steer_b0 = (alpha * alpha) / denom;
...
//       setVoltage(THROTTLE_OUT1, currentOut1, throttleDacResolution);
//       setVoltage(THROTTLE_OUT2, currentOut2, throttleDacResolution);
//       steerVoltage1 = STEER_CENTER;
//       steerVoltage2 = STEER_CENTER;
...
//     if (!manualMode && (command == 'a' || command == 'd')) {
...
//   float pedalPercentage = map(throttle_filtered_value, 140, 750, 0, 100);
//   pedalPercentage = constrain(pedalPercentage, 0, 100);

//   // In manual mode, use rawValue for immediate control
//   if (manualMode) {
//     float pedalVoltage = map(throttleRawValue, 140, 750, idleOut2 * 1000, maxOut2 * 1000) / 1000.0;
...
//   // Steering: Read and filter potentiometer in Manual mode
//   float steerRawValue = 0.0;
//   float steeringAngle = 0.0;
//   if (manualMode) {
//     steerRawValue = analogRead(STEER_POT);
...
//     // Read ADC inputs for DAC control
//     int val1 = analogRead(STEER_ANALOG1);
//     int val2 = analogRead(STEER_ANALOG2);
//     analogWrite(STEER_OUT1, val1 > 0 ? val1 : STEER_IDLE_STATE);
//     analogWrite(STEER_OUT2, val2 > 0 ? val2 : STEER_IDLE_STATE);
//     steerVoltage1 = (val1 / 4095.0) * 3.3;
//     steerVoltage2 = (val2 / 4095.0) * 3.3;
//   }
