...
baud_rate = 9600
output_file = 'gas_pedal_data.csv'
...
with open(output_file, mode='w', newline='') as file:
    ...
    writer.writerow(['Time (ms)', 'Raw ADC Value', 'Gas Pedal Position (%)'])  # Write header
    
    print("Logging gas pedal data with timestamp to CSV...")
    ...
    try:
        while True:
            if ser.in_waiting > 0:
                ...
                if line:
                    data = line.split(',')
                    # Check if the data has exactly 3 elements (time, raw_value, percentage)
                    if len(data) == 3:
                        try:
                            time_ms = float(data[0])  # Ensure time is a number
                            raw_value = float(data[1])  # Ensure raw value is a number
                            gas_percent = float(data[2])  # Ensure percentage is a number
                            
                            # Write the data to CSV
                            writer.writerow([time_ms, raw_value, gas_percent])
                            print(f"Time: {time_ms} ms, Raw ADC Value: {raw_value}, Gas Pedal Position: {gas_percent} %")
                        except ValueError:
