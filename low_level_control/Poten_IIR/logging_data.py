...
with open(output_file, mode='w', newline='') as file:
    ...
    writer.writerow(['Time (ms)', 'Raw ADC Value', 'Steering Angle (%)'])  # Write header
    
    print("Logging steering angle data with timestamp to CSV...")
    ...
    try:
        while True:
            if ser.in_waiting > 0:
                ...
                if line:
                    data = line.split(',')
                    # Check if the data has exactly 3 elements (time, raw_value, steering_angle)
                    if len(data) == 3:
                        try:
                            time_ms = float(data[0])  # Ensure time is a number
                            raw_value = float(data[1])  # Ensure raw value is a number
                            steering_angle = float(data[2])  # Ensure steering angle is a number
                            
                            # Write the data to CSV
                            writer.writerow([time_ms, raw_value, steering_angle])
                            print(f"Time: {time_ms} ms, Raw ADC Value: {raw_value}, Steering Angle: {steering_angle} %")
                        except ValueError:
