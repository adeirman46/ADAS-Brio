...
class NetworkManager:
    def __init__(self):
        ...
        self.delta_socket = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)  # New socket for delta_m
        ...
        self.delta_address = ('localhost', 12361)  # New address for delta_m
    ...
    def cleanup(self):
        ...
        self.delta_socket.close()  # Close the new socket
...
def get_roi_top_y(roi_mask):
    """
    Find the topmost y-coordinate where the ROI mask is non-zero.
    This determines the top of the drivable area within the ROI.
    """
    height, width = roi_mask.shape
    for y in range(height):
        if np.any(roi_mask[y] > 0):
            return y
    return 0  # Fallback to top of image if ROI is empty

def extract_lane_boundaries(drivable_mask, roi_mask=None, scan_end=None):
    """
    Extract leftmost and rightmost boundaries from drivable area segmentation with improved noise handling.
    
    Args:
        drivable_mask: Binary mask of drivable area
        roi_mask: Optional ROI mask to limit processing area
        scan_end: The topmost y-coordinate to scan to (default is based on ROI)
    ...
    # Use provided scan_end or find the top of the ROI
    if scan_end is None:
...
def draw_lane_boundaries(img, left_boundary, right_boundary, distance_points=None):
    """
    Draw lane boundaries and optional distance measurement points.
    
    Args:
        img: Image to draw on
        left_boundary: List of (x, y) points for left boundary
        right_boundary: List of (x, y) points for right boundary
        distance_points: Tuple of (left_point, right_point) to mark with dots
    """
    if left_boundary and len(left_boundary) > 1:
        pts = np.array(left_boundary, dtype=np.int32)
        cv2.polylines(img, [pts], False, (0, 255, 0), 3)
        for point in left_boundary:  # Draw every point for continuity
    ...
    if right_boundary and len(right_boundary) > 1:
        ...
        for point in right_boundary:  # Draw every point for continuity
            cv2.circle(img, (int(point[0]), int(point[1])), 3, (0, 0, 255), -1)
    
    # Draw dots at the points where distances are calculated
    if distance_points:
        left_point, right_point = distance_points
        cv2.circle(img, (int(left_point[0]), int(left_point[1])), 5, (255, 255, 255), -1)  # White dot for left
        cv2.circle(img, (int(right_point[0]), int(right_point[1])), 5, (255, 255, 255), -1)  # White dot for right
...
def calculate_real_world_distances(left_boundary, right_boundary, depth_map, image_width, calc_height=None, calc_distance_m=None):
    """
    Calculate the real-world distances from the image center to the left and right lane boundaries.
    
    Args:
        left_boundary: List of (x, y) points for left boundary
        right_boundary: List of (x, y) points for right boundary
        depth_map: Depth map from ZED camera
        image_width: Width of the image
        calc_height: Percentage of image height (0 to 1) where to calculate distances (optional)
        calc_distance_m: Distance in meters from the bottom to calculate distances (optional)
    
    Returns:
        left_distance_m: Distance to left boundary in meters
        right_distance_m: Distance to right boundary in meters
        distance_points: Tuple of (left_point, right_point) where distances were calculated
    """
    ...
    if left_boundary and right_boundary:
        # Determine the y-coordinate where to calculate distances
        height = max(len(left_boundary), len(right_boundary)) * 5  # Approximate height based on scan_step
        if calc_distance_m is not None:
            # Find the y-coordinate corresponding to calc_distance_m meters from the bottom
            bottom_y = left_boundary[0][1]  # Bottommost y-coordinate
            center_x = image_width // 2
            bottom_depth = depth_map.get_value(center_x, bottom_y)[1]
            
            if np.isfinite(bottom_depth):
                vertical_fov = 60 * (np.pi / 180)  # ZED HD720 vertical FOV ~60 degrees
                tan_half_fov = np.tan(vertical_fov / 2)
                
                # Simplified depth-to-pixel mapping
                target_depth = bottom_depth - calc_distance_m  # Moving up the image (closer to camera)
                if target_depth <= 0:
                    target_depth = bottom_depth / 2  # Fallback to avoid invalid depth
                
                # Map depth to pixel y-coordinate (approximation)
                ...
                calc_y = bottom_y  # Fallback to bottom if depth is invalid
        elif calc_height is not None:
            # Calculate y-coordinate based on percentage of image height
            total_height = left_boundary[0][1] - left_boundary[-1][1]  # Height from bottom to top of boundary
            calc_y = int(left_boundary[0][1] - total_height * calc_height)
        else:
            calc_y = left_boundary[0][1]  # Default to bottommost point
        
        # Find the closest points in the boundaries to calc_y
        left_point = min(left_boundary, key=lambda p: abs(p[1] - calc_y))
        right_point = min(right_boundary, key=lambda p: abs(p[1] - calc_y))
        
        # Calculate distances at these points
        ...
        if np.isfinite(depth_left) and np.isfinite(depth_right):
            fov = 90 * (np.pi / 180)  # Horizontal FOV
...
def make_parser():
    ...
    # Add arguments for scalable distance calculation
    ...
    try:
        while True:
            ...
            try:
                ...
                # Process lane line mask but do not display it
                ...
                # Extract and Smooth Lane Boundaries, extending to the top of the ROI
                left_boundary, right_boundary = extract_lane_boundaries(da_seg_mask, roi_mask)
                
                if left_boundary:
                    left_boundary = smooth_boundary(left_boundary, window_size=3)  # Reduced window size for smoother lines
                if right_boundary:
                ...
                # Calculate Real-World Distances and Steering Direction at specified height/distance
                ...
                if left_distance_m is not None and right_distance_m is not None:
                    ...
                    # Send delta_m to mimo_speed_controller.py
                    network.send_delta(delta_m)

                # Print distances and steering direction
                if left_distance_m is not None and right_distance_m is not None:
            ...
            # Draw Lane Boundaries with distance points
            ...
            if result_img is not None and result_img.size > 0:
                try:
                    ...
                    # Do not display ll_seg_mask as per request
