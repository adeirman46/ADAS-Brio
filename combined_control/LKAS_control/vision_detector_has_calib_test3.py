    ...
    for y in range(scan_start, scan_end, -scan_step):
        ...
        if len(drivable_indices) > 0:
            ...
            if rightmost_x - leftmost_x > min_width:
                # Relax continuity check to allow detection of true leftmost edge
                if left_boundary and abs(leftmost_x - left_boundary[-1][0]) > 100:  # Increased threshold
                    leftmost_x = min(leftmost_x, left_boundary[-1][0])  # Prefer the leftmost position
...
def detect():
    ...
    try:
        while True:
            ...
            # Debug: Visualize the drivable mask overlay
            debug_img = im0.copy()
            debug_img[da_seg_mask > 0] = debug_img[da_seg_mask > 0] * 0.5 + np.array([255, 0, 0], dtype=np.uint8) * 0.5
            cv2.imshow("Drivable Mask Debug", debug_img)
