...
class CARLACamera:
    def __init__(self):
        ...
        self.STOP_DELAY = 10.0  # Seconds before sudden stop
    ...
    def aeb_control(self, distance, threshold=10.0, hard_brake_threshold=5.0):
        """
        Autonomous Emergency Braking control function.
        
        Args:
            distance (float): Distance to obstacle in meters
            threshold (float): Distance threshold to start braking (meters)
            hard_brake_threshold (float): Distance threshold for hard braking (meters)
        ...
        if distance <= hard_brake_threshold:
            ...
            # Gradual braking
            brake_intensity = (threshold - distance) / (threshold - hard_brake_threshold)
            control.throttle = 0.0
            control.brake = min(brake_intensity, 0.8)
    ...
    # def setup_aeb_scenario(self, lead_vehicle_distance=20.0):
    #     """
    #     Setup the AEB test scenario with a lead vehicle.
        
    #     Args:
    #         lead_vehicle_distance (float): Initial distance of lead vehicle from ego vehicle
    #     """
    #     try:
    ...
    #         # Calculate spawn point for lead vehicle
    #         lead_spawn = carla.Transform(
    #             location=ego_transform.location + forward_vector * lead_vehicle_distance,
    ...
    #         self.lead_vehicle = self.world.spawn_actor(vehicle_bp, lead_spawn)
            
    #         # Setup lead vehicle behavior
    #         self.lead_vehicle.set_autopilot(False)
            
    #         return self.lead_vehicle
            
    #     except Exception as e:
    #         print(f"Error setting up AEB scenario: {e}")
    #         return None
    ...
    #             raise RuntimeError("Could not get ego vehicle transform!")
    ...
    #                 x=forward_vector.x * lead_vehicle_distance,
    #                 y=forward_vector.y * lead_vehicle_distance,
    #                 z=forward_vector.z * lead_vehicle_distance
    #             ),
    #             rotation=ego_transform.rotation
    #         )
    ...
    #         lead_vehicle = self.world.spawn_actor(vehicle_bp, lead_spawn)
    #         if not lead_vehicle:
    #             raise RuntimeError("Failed to spawn lead vehicle!")
                
    #         # Store lead vehicle reference
    #         self.lead_vehicle = lead_vehicle
            
    #         # Setup initial velocity (50 km/h = 13.89 m/s)
    #         self.initial_speed = 13.89
    #         lead_vehicle.set_target_velocity(carla.Vector3D(x=self.initial_speed, y=0.0, z=0.0))
            
    #         # Schedule the sudden stop after 10 seconds
    #         def sudden_stop():
    #             if self.lead_vehicle and self.lead_vehicle.is_alive:
    #                 self.lead_vehicle.set_target_velocity(carla.Vector3D(x=0.0, y=0.0, z=0.0))
    #                 print("Lead vehicle sudden stop triggered")
            
    #         self.world.call_at_next_tick(lambda: sudden_stop())
            
    #         print("AEB scenario setup completed successfully")
    #         return lead_vehicle
    ...
    def setup_aeb_scenario(self, lead_vehicle_distance=20.0):
        ...
        try:
            ...
            # Store lead vehicle reference and start time
            self.lead_vehicle = lead_vehicle
    ...
    def check_lead_vehicle_timing(self):
        """
        Check if it's time to stop the lead vehicle.
        Should be called in the main loop (e.g., in grab_frame).
        """
        if (self.lead_vehicle and self.start_time and not self.stop_scheduled and 
            time.time() - self.start_time >= self.STOP_DELAY):
            self.lead_vehicle.set_target_velocity(carla.Vector3D(x=0.0, y=0.0, z=0.0))
            print("Lead vehicle sudden stop triggered")
            self.stop_scheduled = True

    def grab_frame(self):
        """Modified grab_frame to handle AEB control and lead vehicle timing"""
        try:
            # Update spectator position
            self._update_spectator()
            
            # Check lead vehicle timing
            self.check_lead_vehicle_timing()
            ...
            try:
                ...
                # Calculate minimum distance if we have depth data
                if depth is not None:
                    min_distance = np.min(depth[depth > 0]) if depth.size > 0 else float('inf')
                    self.current_min_distance = min_distance
                    
                    # Apply AEB control based on current distance
                    control = self.aeb_control(min_distance)
                    if self.vehicle:
    ...
    def _spawn_npcs(self, num_vehicles=1, num_pedestrians=0):
        """
        Modified NPC spawning for AEB testing scenario.
        Only spawns the lead vehicle that will perform the sudden stop.
        """
        try:
            # Clear any existing NPCs first
            self._clear_existing_actors()
            
            # Setup the lead vehicle for AEB scenario
            lead_vehicle = self.setup_aeb_scenario()
            if lead_vehicle:
                print("Lead vehicle spawned successfully")
            else:
                print("Failed to spawn lead vehicle")

        except Exception as e:
            print(f"Error spawning NPCs for AEB scenario: {e}")
    ...
    #         print("Spawning NPC vehicles and pedestrians...")
    #         self._spawn_npcs(num_vehicles=15, num_pedestrians=10)
                
    #         self.vehicle = self.world.spawn_actor(vehicle_bp, random.choice(spawn_points))
    #         # Setup AEB scenario after vehicle is spawned
    #         self.setup_aeb_scenario()
    ...
    def open(self, host='localhost', port=2000):
        ...
        try:
            ...
            # Now spawn NPC vehicles for AEB testing
            print("Setting up AEB test scenario...")
            self._spawn_npcs(num_vehicles=1, num_pedestrians=0)
    ...
    # def grab_frame(self):
    #     """Modified grab_frame to handle AEB control"""
    #     try:
    #         # Update spectator position
    #         self._update_spectator()
    ...
    #             # If we have a minimum distance measurement, apply AEB
    #             if hasattr(self, 'current_min_distance'):
    #                 control = self.aeb_control(self.current_min_distance)
    #                 if self.vehicle:
    #                     self.vehicle.apply_control(control)
    ...
    #             # Store initial speed (50 km/h = 13.89 m/s)
    #             self.initial_speed = 13.89
    #             lead_vehicle.set_target_velocity(carla.Vector3D(x=self.initial_speed, y=0.0, z=0.0))
    ...
    #                     lead_vehicle.set_target_velocity(carla.Vector3D(x=0.0, y=0.0, z=0.0))
    #                     print("Lead vehicle sudden stop triggered")
                
    #             self.world.call_at_next_tick(lambda: self.world.get_snapshot().timestamp + 10.0, sudden_stop)
