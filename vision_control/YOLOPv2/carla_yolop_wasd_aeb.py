...
class CARLACamera:
    def __init__(self):
        ...
        self.aeb_active = False  # AEB activation flag
        self.front_vehicle = None  # Vehicle in front for the scenario
    
    # def spawn_front_vehicle(self, distance_ahead=20.0):
    #     """
    #     Spawn a vehicle in front of the ego vehicle for the AEB scenario.
    #     Args:
    #         distance_ahead: Distance in meters to spawn the front vehicle.
    #     """
    #     try:
    ...
    #         # Calculate spawn location ahead of the ego vehicle
    #         spawn_location = ego_transform.location + carla.Location(
    #             x=distance_ahead * ego_transform.get_forward_vector().x,
    #             y=distance_ahead * ego_transform.get_forward_vector().y,
    #             z=0
    #         )

    #         # Spawn the front vehicle
    #         self.front_vehicle = self.world.spawn_actor(vehicle_bp, carla.Transform(spawn_location, ego_transform.rotation))
    #         print(f"Spawned front vehicle at {distance_ahead} meters ahead.")

    #         # Make the front vehicle stop after a few seconds
    #         self.world.tick()
    #         time.sleep(5)  # Wait for 5 seconds before stopping
    #         self.front_vehicle.apply_control(carla.VehicleControl(throttle=0.0, brake=1.0))
    #         print("Front vehicle has stopped.")
    ...
    def spawn_front_vehicle(self, distance_ahead=20.0):
        ...
        Args:
            distance_ahead (float): Distance in meters to spawn the front vehicle
            
        Returns:
            bool: True if vehicle was spawned successfully, False otherwise
        """
        try:
            if not self.vehicle:
                print("Error: Ego vehicle not initialized")
                return False
            ...
            # Calculate spawn location ahead of the ego vehicle
            spawn_location = ego_transform.location + carla.Location(
                x=distance_ahead * forward_vector.x,
                y=distance_ahead * forward_vector.y,
                z=0.5  # Slight offset to prevent spawning inside ground
            )
            
            # Create spawn transform
            spawn_transform = carla.Transform(
                spawn_location,
                ego_transform.rotation  # Use same rotation as ego vehicle
            )
            
            # Check if spawn point is clear
            if self.world.cast_ray(spawn_location, spawn_location + carla.Location(z=2.0)):
                print("Spawn location is not clear")
                return False
                
            # Spawn the front vehicle
            self.front_vehicle = self.world.spawn_actor(vehicle_bp, spawn_transform)
            
            if self.front_vehicle is None:
                print("Failed to spawn front vehicle")
                return False
                
            # Set the vehicle to be stationary
            control = carla.VehicleControl()
            control.brake = 1.0
            control.throttle = 0.0
            self.front_vehicle.apply_control(control)
            
            print(f"Successfully spawned front vehicle at {distance_ahead} meters ahead")
            return True
    ...
    def aeb_control(self, distance):
        """
        Automatic Emergency Braking control.
        Args:
            distance: Distance to the vehicle in front (in meters)
        Returns:
            carla.VehicleControl: Control command with velocity set to 0 if AEB is active
        """
        aeb_threshold = 5.0  # Distance threshold for AEB activation (in meters)
        
        if distance < aeb_threshold:
    ...
    def process_input(self):
        ...
        # If AEB is active, skip manual control
        if self.aeb_active:
            return True
    ...
    def open(self, host='localhost', port=2000):
        ...
        try:
            ...
            # Spawn a vehicle in front for the AEB scenario
            self.spawn_front_vehicle(distance_ahead=20.0)
