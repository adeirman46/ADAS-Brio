...
def calibrate_camera(zed):
    # Use fixed coordinates to form a trapezoid
    tl, bl, tr, br = (572, 381), (445, 568), (931, 358), (1224, 535)
    print(f"Using fixed coordinates: TL={tl}, BL={bl}, TR={tr}, BR={br}")
...
def draw_roi(frame, pts1):
    ...
    # Assuming pts1 is [tl, bl, tr, br]
    tl, bl, tr, br = pts1
    # Draw in order: tl -> tr -> br -> bl to form a proper trapezoid
...
def detect():
    ...
    try:
        while True:
            ...
            # Resize lane line mask to match the original frame's dimensions
            ll_seg_mask = cv2.resize(ll_seg_mask, (im0_rgb.shape[1], im0_rgb.shape[0]), 
                                     interpolation=cv2.INTER_NEAREST)

            # Transform lane line mask to bird's-eye view
            ll_seg_mask_transformed = cv2.warpPerspective(ll_seg_mask, matrix, (640, 480), flags=cv2.INTER_NEAREST)

            # Create a dummy drivable area mask (all zeros) to satisfy show_seg_result
            ...
            # Process detections and plot bounding boxes with depth on the original frame
            for i, det in enumerate(pred):
                if len(det):
                    # Rescale boxes to the original frame's dimensions
                    det[:, :4] = scale_coords(img.shape[2:], det[:, :4], im0_rgb.shape).round()
                    # Plot bounding boxes and add depth information
            ...
            # Overlay only the lane line segmentation (blue area) on the transformed frame
            ...
            # Display the original frame with ROI, bounding boxes, and depth, and the transformed results
