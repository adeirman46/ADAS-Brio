...
def pure_pursuit_controller(left_fit, right_fit, vehicle_pos=(320, 480), lookahead_dist=50):
    """Compute steering angle and velocity using Pure Pursuit algorithm"""
    # Default outputs
    steering_angle = 0.0  # in radians
    velocity = 5.0  # constant speed in m/s (can be tuned)

    if left_fit is None or right_fit is None:
        return steering_angle, velocity

    # Compute the centerline between left and right lanes
    ploty = np.linspace(0, 480-1, 480)  # Match BEV height
    left_fitx = left_fit[0]*ploty**2 + left_fit[1]*ploty + left_fit[2]
    right_fitx = right_fit[0]*ploty**2 + right_fit[1]*ploty + right_fit[2]
    ...
    lx, ly = lookahead_point[0] - vehicle_x, lookahead_point[1] - vehicle_y
    ld = math.sqrt(lx**2 + ly**2)
    if ld < 1e-6:  # Avoid division by zero
        return steering_angle, velocity

    # Compute the curvature (2 * lateral offset / lookahead distance^2)
    alpha = math.atan2(lx, -ly)  # Angle to lookahead point (assuming vehicle faces up in BEV)
    curvature = 2 * math.sin(alpha) / ld

    # Steering angle (simplified, assumes wheelbase of 1 for scaling)
    steering_angle = math.atan(curvature)

    # Clip steering angle to reasonable limits (e.g., -30 to 30 degrees)
...
def detect():
    ...
    try:
        while True:
            ...
            da_seg_mask_transformed = np.zeros_like(ll_seg_mask_transformed)
            ...
            # Compute steering angle and velocity using Pure Pursuit
            steering_angle, velocity = pure_pursuit_controller(left_fit, right_fit)
            ...
            # Add FPS, steering angle, and velocity to the result frame
            cv2.putText(result_frame, f'FPS: {fps_avg.avg:.2f}', (20, 40), 
                        cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 0), 2)
            cv2.putText(result_frame, f'Steering: {math.degrees(steering_angle):.2f} deg', (20, 160), 
                        cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 255, 255), 2)
            cv2.putText(result_frame, f'Velocity: {velocity:.2f} m/s', (20, 200), 
                        cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 255, 255), 2)
