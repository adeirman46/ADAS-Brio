...
def draw_polynomial_lines(image, left_fit, right_fit, depth_data, da_seg_mask_transformed, color=(255, 0, 0), thickness=3):
    """Draw polynomial lines on the image and display deviation based on depth"""
    ...
    # Calculate deviation using depth from driving area
    ...
    if len(coords) > 0:
        ...
        # Get depth values (assuming depth_data is the original depth array transformed to 640x480)
        left_depth = depth_data[bottom_y, int(left_x)] if not np.isnan(depth_data[bottom_y, int(left_x)]) else 0.0
        center_depth = depth_data[bottom_y, center_x] if not np.isnan(depth_data[bottom_y, center_x]) else 0.0
        right_depth = depth_data[bottom_y, int(right_x)] if not np.isnan(depth_data[bottom_y, int(right_x)]) else 0.0
        
        # Calculate deviation as the difference from center depth
        deviation = center_depth - (left_depth + right_depth) / 2 if center_depth > 0 else 0.0
