...
def calibrate_camera(zed, calibrate=False):
    if calibrate:
        ...
        while True:
            ...
            if image is None:
                print("Failed to grab frame from ZED camera for calibration.")
                break
...
def draw_polynomial_lines(image, left_fit, right_fit, depth_data, da_seg_mask_transformed, color=(255, 0, 0), thickness=3):
    """Draw polynomial lines on the image and display deviation based on depth, skipping inf values"""
    ...
    # Calculate deviation using depth from driving area, skipping inf values
    center_x = image_width // 2
    bottom_y = image_height - 1
    
    # Find leftmost and rightmost points in driving area before lane lines
    da_mask = da_seg_mask_transformed.astype(bool)
    coords = np.column_stack(np.where(da_mask))
    if len(coords) > 0:
        left_x = np.min(coords[coords[:, 0] <= center_x][:, 1]) if np.any(coords[:, 0] <= center_x) else center_x
        right_x = np.max(coords[coords[:, 0] > center_x][:, 1]) if np.any(coords[:, 0] > center_x) else center_x
        
        # Get depth values, skipping inf and nan
        left_depth = depth_data[bottom_y, int(left_x)]
        center_depth = depth_data[bottom_y, center_x]
        right_depth = depth_data[bottom_y, int(right_x)]
        
        # Check for inf and nan values
        if (not np.isnan(left_depth) and not np.isinf(left_depth) and
            not np.isnan(center_depth) and not np.isinf(center_depth) and
            not np.isnan(right_depth) and not np.isinf(right_depth) and
            center_depth > 0):
            deviation = center_depth - (left_depth + right_depth) / 2
            # Display deviation only if it's a valid number
            deviation_text = f"Deviation: {deviation:.2f} m"
            cv2.putText(image, deviation_text, (20, 160), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 255, 255), 2)
...
def detect():
    ...
    try:
        while True:
            ...
            depth_transformed = cv2.warpPerspective(depth, matrix, (640, 480), flags=cv2.INTER_NEAREST)
            ...
            for i, det in enumerate(pred):
                if len(det):
                    ...
                    for *xyxy, conf, cls in reversed(det):
                        ...
                        # Display depth only if it's not nan or inf
            ...
            # Fit polynomial lanes
            left_fit, right_fit = fit_polynomial(ll_seg_mask_transformed)
            result_frame = draw_polynomial_lines(result_frame, left_fit, right_fit, depth_transformed, da_seg_mask_transformed, color=(255, 0, 0), thickness=3)

            # Determine and display curvature
            curvature_text = "Straight"
            if left_fit is not None and abs(left_fit[0]) > 1e-4:
                curvature_text = "Left" if left_fit[0] < 0 else "Right"
            elif right_fit is not None and abs(right_fit[0]) > 1e-4:
                curvature_text = "Left" if right_fit[0] < 0 else "Right"
            cv2.putText(result_frame, f"Curvature: {curvature_text}", (20, 200), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 255, 255), 2)
