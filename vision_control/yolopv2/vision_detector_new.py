...
class NetworkManager:
    def __init__(self):
        ...
        self.distance_address = ('localhost', 12345)  # For speed controller
        self.gui_distance_address = ('localhost', 12348)  # For GUI
...
def detect():
    ...
    sort_tracker = Sort(max_age=10, min_hits=3, iou_threshold=0.45)
    ...
    try:
        while True:
            ...
            # Update SORT tracker
            tracked_objects = sort_tracker.update(np.array(detections) if detections else np.empty((0, 5)))
            ...
            try:
                ...
                ll_seg_mask = cv2.resize(ll_seg_mask, (im0.shape[1], im0.shape[0]), 
                                       interpolation=cv2.INTER_NEAREST)
                print("Segmentation processed successfully")
            except Exception as e:
            ...
            for obj in tracked_objects:
                ...
                cv2.putText(result_img, f'ID: {int(track_id)}', (x1, y1-10), 
                          cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 0), 2)

            # Add segmentation visualization with safety checks
            if result_img is not None and result_img.size > 0:
                print(f"Result image shape before seg: {result_img.shape}")
                try:
                    ...
                    result_img = seg_img
                    print(f"Result image shape after seg: {result_img.shape}")
                except Exception as e:
                    print(f"Error in segmentation visualization: {e}")
            else:
                print("Error: Invalid result image before segmentation")
            ...
            network.send_tracked_objects(tracked_objects, depth)
...
if __name__ == '__main__':
    try:
        print("Starting YOLOPv2 detection with ZED camera...")
        with torch.no_grad():
            detect()
